      ...
      TaggedTemplateExpression(node) {
        ...
        Object.keys(tagsToCheck).forEach(tag => {
          ...
          while ((match = tagRegex.exec(rawText)) !== null) {
            const [fullMatch, rawTestIdValue] = match;

            const testIdRegex = new RegExp(`<${tag}[^>]*data-testid="([^"]+)"`);
            const testIdMatch = testIdRegex.exec(match[0]);

            // Usunięcie nowej linii i zbędnych spacji z wartości
            const testIdValue = rawTestIdValue.replace(/\s+/g, '');

            // Początek dokładnie od 'ta-testid'
